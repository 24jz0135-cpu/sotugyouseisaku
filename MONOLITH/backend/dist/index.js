...
app.post("/entries", async (c) => {
  ...
  ).bind(type, amount, category ?? null, description ?? null).first();
  ...
  if (image_url || ocr_raw_text) {
    ...
    ).bind(entryId, image_url ?? null, ocr_raw_text ?? null).run();
  }
...
app.get("/analytics/monthly", async (c) => {
  const { year = (/* @__PURE__ */ new Date()).getFullYear(), month = (/* @__PURE__ */ new Date()).getMonth() + 1 } = c.req.query();
